			...
			html2canvas(document.getElementsByClassName("legend")[0],{allowTaint: true,}).then(function(canvas) {
				...
				canv.toBlob(function(blob) { saveAs(blob, "imagen.png"); });
      			...
      			html2canvas(document.getElementsByClassName("info")[0],{allowTaint: true,}).then(function(canvas) {
        			...
        			// Ahora, al segundo parámetro de drawImage le dices que te lo dibuje a la derecha del todo restándole
				// al ancho del mapa el ancho de la leyenda con el nombre de la enfermedad (canvas.width),
				// menos 10 píxeles para que deje un pequeño margen
        			canv.getContext("2d").drawImage(canvas, w3 - canvas.width - 10, 10);
        			canv.toBlob(function(blob) { saveAs(blob, "imagen.png"); });
      			});
			html2canvas(document.getElementsByClassName("copyright")[0],{allowTaint: true,}).then(function(canvas) {
        			var w3 = canv.width;    // Este es el ancho del mapa
				var h2 = canv.height;	// Este es la altura del mapa
        			// Ahora, al segundo parámetro de drawImage le dices que te lo dibuje a la derecha del todo restándole
				// al ancho del mapa el ancho de la leyenda con el nombre de la enfermedad (canvas.width),
				// menos 10 píxeles para que deje un pequeño margen
        			canv.getContext("2d").drawImage(canvas, w3 - canvas.width - 10, h2 - canvas.height - 10);
        			canv.toBlob(function(blob) { saveAs(blob, "imagen.png"); });
      			});
